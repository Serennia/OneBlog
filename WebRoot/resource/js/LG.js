...
//    	 alert();
//    	 alert(JSON.stringify(grid.get("parms")));
    };
    //创建表单搜索按钮：搜索、高级搜索
    ...
    //快速设置表单底部默认的按钮:保存、取消
    LG.setFormDefaultBtn = function (cancleCallback, savedCallback,printCallback)
    {
        //表单底部按钮
        var buttons = [];
        if (cancleCallback)
    ...
    LG.addFormButtons = function (buttons)
    {
        ...
        if (formbar.length == 0)
            formbar = $('<div class="form-bar" style="top:auto;" ><div class="form-bar-inner"></div></div>').appendTo('body');
    ...
    LG.loadToolbar = function (grid, toolbarBtnItemClick)
    {
        ...
        LG.ajax({
            loading: '正在加载工具条中...',
            url: basepath+'manager/getMyButtons',
    ...
    LG.closeAndReloadParent = function (tabid, parentMenuNo)
    {
        ...
        var menuitem = $("#mainmenu ul.l-children li[menuno=" + parentMenuNo + "]");
